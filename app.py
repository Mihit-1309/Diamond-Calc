...
st.set_page_config(page_title="💎 Diamond Price Calculator", page_icon="💎")
...
df = st.data_editor(empty_data, num_rows="dynamic", use_container_width=True, key="diamond_table")
...
if not df.empty and df["weight"].sum() > 0:
    ...
    st.success(f"**Weighted Avg Price per Carat:** ₹ {weighted_avg_price:,.2f}")
else:
    st.info("Please enter at least one row of data above to see results.")
